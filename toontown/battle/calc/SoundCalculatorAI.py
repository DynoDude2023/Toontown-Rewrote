...
class SoundCalculatorAI(DirectObject):
    ...
    def calcAttackResults(self, attack, toonId):
        ...
        for target in targetList:
            ...
            organicBonus = toon.checkGagBonus(atkTrack, atkLevel)
            if organicBonus:
                bonusDamage = target.getActualLevel() * 3 if target.getSkelecog() else target.getActualLevel() * 2 
                attack[TOON_HPBONUS_COL][suits.index(target)] = attack[TOON_HPBONUS_COL][suits.index(target)] + bonusDamage
