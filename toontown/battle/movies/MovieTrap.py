...
def doTraps(traps):
    ...
    for trapList in suitTrapLists:
        ...
        for i in xrange(len(trapList)):
            ...
            elif level == ToontownBattleGlobals.RAILROAD_LEVEL_INDEX:
        ...
        if len(trapList) == 1 and not hasUberTrapConflict:
            interval = __doTrapLevel(trapList[0], trapPropList[0])
            if interval:
                mtrack.append(interval)
        else:
            subMtrack = Parallel()
            for i in xrange(len(trapList)):
                trap = trapList[i]
                trapProps = trapPropList[i]
                interval = __doTrapLevel(trap, trapProps, explode=1)
                if interval:
                    subMtrack.append(interval)
...
def __createThrownTrapMultiTrack(trap, propList, propName, propPos=None, propHpr=None, anim=0, explode=0):
    ...
    if trapName == 'banana':
        ...
        moveTrack = Sequence(Func(thrownProp.wrtReparentTo, suit), Func(thrownProp.setHpr, Point3(94, 0, 0)),
...
def createThrowingTrack(object, target, duration=1.0, parent=render, gravity=-32.144):
    ...
    return Sequence(Func(calcOriginAndVelocity),
                    LerpFunctionInterval(throwPos, fromData=0.0, toData=1.0, duration=duration,
