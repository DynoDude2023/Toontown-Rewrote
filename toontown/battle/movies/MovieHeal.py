...
soundFiles = (
    'AA_heal_tickle.ogg', 'AA_heal_telljoke.ogg', 'AA_heal_smooch.ogg', 'AA_heal_happydance.ogg',
    'AA_heal_pixiedust.ogg',
    'AA_heal_juggle.ogg', 'AA_heal_pixiedust.ogg', 'AA_heal_High_Dive.ogg')
...
def __healTickle(heal):
    ...
    mtrack = Parallel(featherTrack, ActorInterval(toon, 'tickle'),
                      ...
                      Sequence(Wait(tHeal), Func(__healToon, targetToon, hp, ineffective),
...
def __healSmooch(heal):
    ...
    mtrack = Parallel(lipstickTrack, lipsTrack, __getSoundTrack(level, 2, node=toon),
                      ...
                      Sequence(Wait(delay), Func(__healToon, targetToon, hp, ineffective)))
    track.append(mtrack)
    track.append(Func(targetToon.clearChat))
    return track
