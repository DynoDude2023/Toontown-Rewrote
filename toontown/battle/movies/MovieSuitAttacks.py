...
from toontown.battle.MovieSuitAttacks import doAdvancedSynergy
...
def doClipOnTie(attack):
    ...
    tiePropTrack = Sequence(
        getPropAppearTrack(tie, suit.getRightHand(), posPoints, 0.5, MovieUtil.PNT3_ONE, scaleUpTime=0.5,
    ...
    tiePropTrack.append(
        getPropThrowTrack(attack, tie, [__toonFacePoint(toon)], [__toonGroundPoint(attack, toon, 0.1)], hitDuration=0.4,
...
def doPoundKey(attack):
    ...
    propTrack = Sequence(Wait(0.3), Func(__showProp, phone, suit.getLeftHand(), phonePosPoints[0], phonePosPoints[1]),
                         Func(__showProp, receiver, suit.getLeftHand(), receiverPosPoints[0], receiverPosPoints[1]),
                         LerpScaleInterval(phone, 0.5, MovieUtil.PNT3_ONE, MovieUtil.PNT3_NEARZERO), Wait(0.74),
                         ...
                         Wait(3.14), Func(receiver.wrtReparentTo, phone), Wait(0.62),
...
def doShred(attack):
    ...
    paperPropTrack = getPropTrack(paper, suit.getRightHand(), paperPosPoints, 2.4, 1e-05, scaleUpTime=0.2, anim=1,
                                  propName='shredder-paper', animDuration=1.5, animStartTime=2.8)
    shredderPosPoints = [Point3(0, -0.12, -0.34), VBase3(-90.0, -53.77, -0.0)]
    shredderPropTrack = getPropTrack(shredder, suit.getLeftHand(), shredderPosPoints, 1, 3,
                                     scaleUpPoint=Point3(4.81, 4.81, 4.81))
    toonTrack = getToonTrack(attack, suitTrack.getDuration() - 1.1, ['conked'], suitTrack.getDuration() - 3.1,
                             ['sidestep'])
...
def doFillWithLead(attack):
    ...
    soundTrack = getSoundTrack('SA_fill_with_lead.ogg', delay=startDelay, node=suit)
    ...
    appearDelay = 1.3
    sharpenerPropTrack = getPropTrack(sharpener, suit.getLeftHand(), sharpenerPosPoints, appearDelay, 2.3,
    ...
    toonTrack = getToonTrack(attack, splicedDamageAnims=damageAnims, dodgeDelay=suitTrack.getDuration() - 3.1,
                             dodgeAnimNames=['sidestep'], showDamageExtraTime=4.5, showMissedExtraTime=1.6)
    ...
    if dmg > 0:
        ...
        return Parallel(suitTrack, pencilPropTrack, sharpenerPropTrack, sprayTrack, soundTrack, headTrack,
                        torsoTrack, legsTrack, colorTrack, toonTrack)
    else:
        return Parallel(suitTrack, pencilPropTrack, sharpenerPropTrack, sprayTrack, soundTrack, toonTrack)
...
def doFountainPen(attack):
    ...
    hitSprayTrack = MovieUtil.getSprayTrack(battle, VBase4(0, 0, 0, 1), getPenTip, hitPoint, 0.2, 0.2, 0.2,
                                            horizScale=0.1, vertScale=0.1)
    missSprayTrack = MovieUtil.getSprayTrack(battle, VBase4(0, 0, 0, 1), getPenTip, missPoint, 0.2, 0.2, 0.2,
                                             horizScale=0.1, vertScale=0.1)
    suitTrack = getSuitTrack(attack)
    propTrack = Sequence(Wait(0.01), Func(__showProp, pen, suit.getRightHand(), MovieUtil.PNT3_ZERO),
                         LerpScaleInterval(pen, 0.5, Point3(1.5, 1.5, 1.5)), Wait(1.05))
    if dmg > 0:
    ...
    if dmg > 0:
        ...
        splashTrack = Sequence(Func(battle.movie.needRestoreRenderProp, splash), Wait(1.65),
...
def doRubOut(attack):
    ...
    if dmg > 0:
        ...
        return Parallel(suitTrack, toonTrack, padPropTrack, pencilPropTrack, soundTrack, hideTrack, headTrack,
                        torsoTrack, legsTrack)
    else:
...
def doWriteOff(attack):
    ...
    pencilPropTrack = Sequence(Wait(0.5), Func(__showProp, *extraArgsForShowProp),
                               LerpScaleInterval(pencil, 0.5, Point3(1.5, 1.5, 1.5), startScale=Point3(0.01)), Wait(2),
                               Func(battle.movie.needRestoreRenderProp, checkmark), Func(checkmark.reparentTo, render),
                               Func(checkmark.setScale, 1.6), Func(checkmark.setPosHpr, pencil, 0, 0, 0, 0, 0, 0),
    ...
    soundTrack = Sequence(Wait(2.3),
                          SoundInterval(globalBattleSoundCache.getSound('SA_writeoff_pen_only.ogg'), duration=0.9,
                                        node=suit),
...
def doRubberStamp(attack):
    ...
    propTrack = Sequence(Func(__showProp, stamp, suit.getRightHand(), stampPosPoints[0], stampPosPoints[1]),
                         LerpScaleInterval(stamp, 0.5, MovieUtil.PNT3_ONE), Wait(2.6),
                         Func(battle.movie.needRestoreRenderProp, cancelled), Func(cancelled.reparentTo, render),
                         Func(cancelled.setScale, 0.6), Func(cancelled.setPosHpr, stamp, 0.81, -1.11, -0.16, 0, 0, 90),
                         Func(cancelled.setP, 0), Func(cancelled.setR, 0))
...
def doRazzleDazzle(attack):
    ...
    if hitSuit:
        ...
        hitPoint = lambda particleEffect=particleEffect, toon=toon, suit=suit: __toonMissPoint(particleEffect, toon,
                                                                                               parent=suit.getRightHand())
    signPropTrack = Sequence(Wait(0.5), Func(__showProp, sign, suit.getRightHand(), signPosPoints[0], signPosPoints[1]),
                             LerpScaleInterval(sign, 0.5, Point3(1.39, 1.39, 1.39)), Wait(0.5),
                             Func(battle.movie.needRestoreParticleEffect, particleEffect),
                             Func(particleEffect.start, sign), Func(particleEffect.wrtReparentTo, render),
                             LerpPosInterval(particleEffect, 2.0, pos=hitPoint), Func(particleEffect.cleanup),
    ...
    return Sequence(Parallel(suitTrack, signPropTrack, signPropAnimTrack, toonTrack, soundTrack),
...
def doTeeOff(attack):
    ...
    if suitName == 'ym':
        ...
        ballPosPoints = [Point3(2.1, 0, 0.1)]
    ballPropTrack = Sequence(getPropAppearTrack(ball, suit, ballPosPoints, 1.7, Point3(1.5, 1.5, 1.5)),
    ...
    toonTrack = getToonTrack(attack, suitTrack.getDuration() - 2.25, ['conked'], dodgeDelay, ['duck'],
...
def doBrainStorm(attack):
    ...
    cloudPropTrack.append(Parallel(ParticleInterval(snowEffect, cloud, worldRelative=0, duration=2.2, cleanup=True),
                                   Sequence(Wait(0.5),
                                            ParticleInterval(snowEffect2, cloud, worldRelative=0, duration=1.7,
                                                             cleanup=True)), Sequence(Wait(1.0),
                                                                                      ParticleInterval(snowEffect3,
                                                                                                       cloud,
                                                                                                       worldRelative=0,
                                                                                                       duration=1.2,
                                                                                                       cleanup=True)),
                                   Sequence(ActorInterval(cloud, 'stormcloud', startTime=3, duration=0.5),
...
def doBuzzWord(attack):
    ...
    toonTrack = getToonTrack(attack, damageDelay=damageDelay, damageAnimNames=['cringe'],
                             splicedDodgeAnims=[['duck', dodgeDelay, 1.4]], showMissedExtraTime=dodgeDelay + 0.5)
...
def doDemotion(attack):
    ...
    toonTrack = getToonTrack(attack, damageDelay=1.0, splicedDamageAnims=damageAnims, splicedDodgeAnims=dodgeAnims,
                             showMissedExtraTime=1.6, showDamageExtraTime=1.3)
...
def doCanned(attack):
    ...
    throwTrack = Sequence(
        getPropAppearTrack(can, suit.getRightHand(), posPoints, propDelay, Point3(6, 6, 6), scaleUpTime=0.5))
    ...
    if dmg > 0:
        ...
        hprTrack = Sequence(Wait(propDelay + suitDelay), LerpHprInterval(can, throwDuration, canHpr), Wait(0.4),
    ...
    canTrack = Sequence(Parallel(throwTrack, scaleTrack, hprTrack), Func(MovieUtil.removeProp, can),
                        Func(battle.movie.clearRenderProp, can))
    damageAnims = [['struggle',
                    propDelay + suitDelay + throwDuration,
                    0.01,
                    0.7], ['slip-backward', 0.01, 0.45]]
    toonTrack = getToonTrack(attack, splicedDamageAnims=damageAnims, dodgeDelay=dodgeDelay, dodgeAnimNames=['sidestep'],
...
def doDownsize(attack):
    ...
    if dmg > 0:
        ...
        shrinkTrack = Sequence(Wait(damageDelay + 0.5), Func(battle.movie.needRestoreToonScale),
                               ...
                               LerpScaleInterval(toon, 0.1, downScale * 0.95), LerpScaleInterval(toon, 0.1, downScale),
                               Wait(2.1), LerpScaleInterval(toon, 0.5, initialScale * 1.5),
                               ...
                               LerpScaleInterval(toon, 0.15, initialScale), Func(battle.movie.clearRestoreToonScale))
    damageAnims = []
    ...
    damageAnims.append(['sidestep-right',
                        ...
                        2.97,
                        1.49])
    toonTrack = getToonTrack(attack, damageDelay=damageDelay, splicedDamageAnims=damageAnims, dodgeDelay=0.6,
...
def doPinkSlip(attack):
    ...
    if dmg > 0:
        ...
        paperSpinTrack = Sequence(Wait(throwDelay), LerpHprInterval(paper, throwDuration, VBase3(300, 0, 0)),
                                  Wait(paperPause), LerpHprInterval(paper, 1.3, VBase3(-200, 100, 100)))
    else:
    ...
    toonTrack = getToonTrack(attack, damageDelay=2.81, splicedDamageAnims=damageAnims, dodgeDelay=2.8,
                             dodgeAnimNames=['jump'], showDamageExtraTime=0.9)
...
def doExecutivePinkSlip(attack):
    suit = attack['suit']
    battle = attack['battle']
    target = attack['target']
    toon = target['toon']
...
def doSacked(attack):
    ...
    sackAppearTrack = Sequence(
        getPropAppearTrack(sack, suit.getRightHand(), posPoints, propDelay, initialScale, scaleUpTime=0.2))
    ...
    if dmg > 0:
        ...
        scaleTrack = Sequence(Wait(propDelay + suitDelay), LerpScaleInterval(sack, throwDuration, scaleUpPoint),
                              Wait(1.8), LerpScaleInterval(sack, 0.3, MovieUtil.PNT3_NEARZERO))
        hprTrack = Sequence(Wait(propDelay + suitDelay), LerpHprInterval(sack, throwDuration, sackHpr))
        sackTrack = Sequence(Parallel(sackAppearTrack, scaleTrack, hprTrack), Func(MovieUtil.removeProp, sack),
        ...
        sackTrack = Sequence(sackAppearTrack, Func(MovieUtil.removeProp, sack),
                             Func(battle.movie.clearRenderProp, sack))
    damageAnims = [['struggle',
                    0.01,
                    0.01,
                    0.7], ['slip-backward', 0.01, 0.45]]
    toonTrack = getToonTrack(attack, damageDelay=propDelay + suitDelay + throwDuration, splicedDamageAnims=damageAnims,
                             dodgeDelay=3.0, dodgeAnimNames=['sidestep'], showDamageExtraTime=1.8,
...
def doGlowerPower(attack):
    ...
    for i in xrange(0, 3):
        ...
        leftTrack.append(
            getPropThrowTrack(attack, leftKnives[i], hitPointNames=['face'], missPointNames=['miss'], hitDuration=0.3,
                              missDuration=0.3))
        ...
        rightTrack.append(
            getPropThrowTrack(attack, rightKnives[i], hitPointNames=['face'], missPointNames=['miss'], hitDuration=0.3,
                              missDuration=0.3))
        rightKnifeTracks.append(rightTrack)

    damageAnims = [['slip-backward', 0.01, 0.35]]
    toonTrack = getToonTrack(attack, damageDelay=1.6, splicedDamageAnims=damageAnims, dodgeDelay=0.7,
                             dodgeAnimNames=['sidestep'])
...
def doHalfWindsor(attack):
    ...
    tiePropTrack.append(
        getPropThrowTrack(attack, tie, [hitPoint], [missPoint], hitDuration=0.4, missDuration=0.8, missScaleDown=0.3,
                          parent=battle))
    damageAnims = [['conked',
                    0.01,
                    0.01,
    ...
    throwSound = getSoundTrack('SA_half_windsor_throw.ogg', delay=2.3, node=suit)
    return Parallel(suitTrack, toonTrack, tiePropTrack, throwSound)
...
def doHeadShrink(attack):
    ...
    if dmg > 0:
        ...
        soundTrack = Sequence(Wait(2.1), SoundInterval(shrinkSound, duration=2.1, node=suit), Wait(1.6),
...
def doRolodex(attack):
    ...
    suitTrack = getSuitTrack(attack)
    propTrack = getPropTrack(rollodex, suit.getLeftHand(), propPosPoints, 1e-06, 4.7, scaleUpPoint=propScale, anim=0,
                             propName='rollodex', animDuration=0, animStartTime=0)
    toonTrack = getToonTrack(attack, damageDelay, ['conked'], dodgeDelay, ['sidestep'])
...
def doEvilEye(attack):
    ...
    if suitName == 'cr':
        ...
        posPoints = [Point3(-0.4, 3.45, 3.4), VBase3(-155.0, -20.0, 0.0)]
    else:
        posPoints = [Point3(-0.4, 3.65, 5.01), VBase3(-155.0, -20.0, 0.0)]
    appearDelay = 0.8
    suitHoldStart = 1.06
    ...
    eyeAppearTrack = Sequence(Wait(suitHoldStart), Func(__showProp, eye, suit, posPoints[0], posPoints[1]),
                              LerpScaleInterval(eye, suitHoldDuration, Point3(11, 11, 11)), Wait(eyeHoldDuration * 0.3),
                              LerpHprInterval(eye, 0.02, Point3(205, 40, 0)), Wait(eyeHoldDuration * 0.7),
                              Func(battle.movie.needRestoreRenderProp, eye), Func(eye.wrtReparentTo, battle))
    toonFace = __toonFacePoint(toon, parent=battle)
    if dmg > 0:
        lerpInterval = LerpPosInterval(eye, moveDuration, toonFace)
    else:
        lerpInterval = LerpPosInterval(eye, moveDuration,
    ...
    eyePropTrack = Sequence(eyeAppearTrack, Parallel(eyeMoveTrack, eyeRollTrack),
                            Func(battle.movie.clearRenderProp, eye), Func(MovieUtil.removeProp, eye))
    damageAnims = [['duck',
                    0.01,
                    0.01,
                    1.4], ['cringe', 0.01, 0.3]]
...
def doPlayHardball(attack):
    ...
    propTrack = Sequence(
        getPropAppearTrack(ball, suit.getRightHand(), ballPosPoints, 0.8, Point3(5, 5, 5), scaleUpTime=0.5))
...
def doPowerTie(attack):
    ...
    tiePropTrack = Sequence(
        getPropAppearTrack(tie, suit.getRightHand(), posPoints, 0.5, Point3(3.5, 3.5, 3.5), scaleUpTime=0.5))
    tiePropTrack.append(Wait(throwDelay))
...
def doDoubleTalk(attack):
    ...
    soundTrack = getSoundTrack('SA_doubletalk.ogg', delay=2.5, node=suit)
...
def doHotAir(attack):
    ...
    if dmg > 0:
        return Parallel(suitTrack, toonTrack, sprayTrack, soundTrack, baseFlameTrack, flameTrack, flecksTrack,
                        colorTrack)
    else:
...
def doSchmooze(attack):
    ...
    for i in xrange(0, 4):
        ...
        lowerPartTracks.append(
            getPartTrack(lowerEffects[i], partDelay + i * 0.65 + 0.7, 1.0, [lowerEffects[i], suit, 0]))
    ...
    toonTrack = getToonTrack(attack, damageDelay=damageDelay, splicedDamageAnims=damageAnims, dodgeDelay=dodgeDelay,
                             splicedDodgeAnims=dodgeAnims, showMissedExtraTime=1.9, showDamageExtraTime=1.1)
...
def doShake(attack):
    ...
    toonTracks = getToonTracks(attack, damageDelay=1.1, splicedDamageAnims=damageAnims, dodgeDelay=0.7,
                               splicedDodgeAnims=dodgeAnims, showMissedExtraTime=2.8, showDamageExtraTime=1.1)
    soundTrack = getSoundTrack('SA_shake.ogg', node=suit)
    return Parallel(suitTrack, toonTracks)

def doRoboRepair(attack):
    suit = attack['suit']
    battle = attack['battle']
    suitTrack = getSuitAnimTrack(attack)
    suits = battle.activeSuits
    otherCogs = []
    repairSeqHeals = Parallel()
    repairSfx = base.loader.loadSfx("phase_5/audio/sfx/SA_repair.ogg")
    for cog in suits:
        if cog != suit:
            otherCogs.append(cog)
            for healingSuit in otherCogs:    
                chatTrack = Sequence(Func(healingSuit.setChatAbsolute, random.choice(['Needed.', "Thank you."]), CFSpeech | CFTimeout))
                repairSeqHeal = Sequence(Wait(3), chatTrack, Func(healingSuit.setHP, healingSuit.getHP() + healingSuit.getMaxHP() / 2), Func(healingSuit.updateHealthBar, 0), Func(healingSuit.showHpText, healingSuit.getMaxHP() / 2))
                repairSeqHeals.append(repairSeqHeal)
        else:
            chatTrack = Sequence()
            repairSeqHeal = Sequence(Wait(3), chatTrack, Func(cog.setHP, cog.getHP() + cog.getMaxHP() / 2), Func(repairSfx.play), Func(cog.updateHealthBar, 0), Func(cog.showHpText, cog.getMaxHP() / 2))
            repairSeqHeals.append(repairSeqHeal)
    repairSeq = Parallel(suitTrack, repairSeqHeals)
    return repairSeq
...
def doHangUp(attack):
    ...
    propTrack = Sequence(Wait(0.3), Func(__showProp, phone, suit.getLeftHand(), phonePosPoints[0], phonePosPoints[1]),
                         Func(__showProp, receiver, suit.getLeftHand(), receiverPosPoints[0], receiverPosPoints[1]),
                         LerpScaleInterval(phone, 0.5, scaleUpPoint, MovieUtil.PNT3_NEARZERO), Wait(pickupDelay),
                         Func(receiver.wrtReparentTo, suit.getRightHand()),
                         LerpScaleInterval(receiver, 0.01, receiverAdjustScale),
                         LerpPosHprInterval(receiver, 0.0001, Point3(-0.53, 0.21, -0.54), VBase3(-99.49, -35.27, 1.84)),
                         Wait(dialDuration), Func(receiver.wrtReparentTo, phone), Wait(finalPhoneDelay),
                         LerpScaleInterval(phone, 0.5, MovieUtil.PNT3_NEARZERO),
                         Func(MovieUtil.removeProps, [receiver, phone]))
...
def doParadigmShift(attack):
    ...
    toonTracks = getToonTracks(attack, damageDelay=damageDelay, splicedDamageAnims=damageAnims, dodgeDelay=dodgeDelay,
                               splicedDodgeAnims=dodgeAnims, showDamageExtraTime=2.7)
...
def doPowerTrip(attack):
    ...
    def getPowerTrack(effect, suit=suit, battle=battle):
        partTrack = Sequence(Wait(1.0), Func(battle.movie.needRestoreParticleEffect, effect), Func(effect.start, suit),
                             Wait(0.4), LerpPosInterval(effect, 1.0, Point3(0, 15, 0.4)),
    ...
    for target in targets:
        if target['hp'] > 0:
            soundTrack = getSoundTrack('SA_powertrip.ogg', startDelay)
            hitAtleastOneToon = 1
            break
    if not hitAtleastOneToon:
        soundTrack = getSoundTrack('SA_powertrip.ogg', startDelay, 0.35)

    waterfallTrack = getPartTrack(waterfallEffect, startDelay, 1.3, [waterfallEffect, suit, 0])
...
def doBounceCheck(attack):
    ...
    checkPropTrack = Sequence(
        getPropAppearTrack(check, suit.getRightHand(), checkPosPoints, 1e-05, Point3(8.5, 8.5, 8.5),
    ...
    toonTrack = getToonTrack(attack, damageDelay, ['conked'], dodgeDelay, ['sidestep'])
    soundTracks = Sequence(getSoundTrack('SA_pink_slip.ogg', delay=throwDelay + 0.5, duration=0.6, node=suit),
...
def doFired(attack):
    ...
    toonTrack = getToonTrack(attack, damageDelay=1.5, splicedDamageAnims=damageAnims, dodgeDelay=0.3,
                             dodgeAnimNames=['sidestep'])
...
def doBurnTwo(attack):
    ...
    def changeColor(parts):
        ...
        for partNum in xrange(0, parts.getNumPaths()):
            nextPart = parts.getPath(partNum)
            track.append(Func(nextPart.setColorScale, Vec4(0.8, 0.3, 0.3, 1)))
...
def doAudit(attack):
    ...
    calcPropTrack = getPropTrack(calculator, suit.getLeftHand(), calcPosPoints, 1e-06, calcDuration,
...
def doCalculate(attack):
    ...
    soundTrack = getSoundTrack('SA_calculate.ogg', delay=1.9, node=suit)
    return Parallel(suitTrack, toonTrack, calcPropTrack, soundTrack,
                    partTrack, partTrack2, partTrack3, partTrack4, partTrack5)
...
def doTabulate(attack):
    ...
    calcPropTrack = getPropTrack(calculator, suit.getLeftHand(), calcPosPoints, 1e-06, calcDuration,
                                 ...
                                 animDuration=3.4)
    toonTrack = getToonTrack(attack, 3.2, ['conked'], 1.8, ['sidestep'])
    soundTrack = getSoundTrack('SA_tabulate.ogg', delay=1.9, node=suit)
...
def doCrunch(attack):
    ...
    for i in xrange(0, numOfNumbers):
        ...
        numberTrack = Sequence(Wait(0.9), LerpScaleInterval(next, 0.6, MovieUtil.PNT3_ONE), Wait(1.7),
    ...
    toonTrack = getToonTrack(attack, damageDelay=4.7, splicedDamageAnims=damageAnims, dodgeDelay=3.6,
                             dodgeAnimNames=['sidestep'])
...
def doLiquidate(attack):
    ...
    cloudPropTrack.append(
        Parallel(Sequence(ParticleInterval(rainEffect, cloud, worldRelative=0, duration=2.1, cleanup=True)),
                 Sequence(Wait(0.1), ParticleInterval(rainEffect2, cloud, worldRelative=0, duration=2.0, cleanup=True)),
                 Sequence(Wait(0.1), ParticleInterval(rainEffect3, cloud, worldRelative=0, duration=2.0, cleanup=True)),
                 Sequence(ActorInterval(cloud, 'stormcloud', startTime=3, duration=0.1),
    ...
    if dmg > 0:
        ...
        puddleTrack = Sequence(Func(battle.movie.needRestoreRenderProp, puddle), Wait(damageDelay - 0.7),
                               Func(puddle.reparentTo, battle), Func(puddle.setPos, toon.getPos(battle)),
...
def doMarketCrash(attack):
    ...
    toonTrack = getToonTrack(attack, damageDelay=3.8, splicedDamageAnims=damageAnims, dodgeDelay=2.4,
...
def doBite(attack):
    ...
    if dmg > 0:
        ...
        scaleTrack = Sequence(Wait(throwDelay), LerpScaleInterval(teeth, throwDuration, Point3(8, 8, 8)), Wait(0.9),
                              LerpScaleInterval(teeth, 0.2, Point3(14, 14, 14)), Wait(1.2),
        ...
        soundTrack = getSoundTrack('SA_bite.ogg', delay=throwDelay, node=suit)
    else:
    ...
    dodgeAnims = [['cringe',
                   ...
                   0.7,
                   0.2], ['duck', 0.01, 1.6]]
    toonTrack = getToonTrack(attack, damageDelay=3.2, splicedDamageAnims=damageAnims, dodgeDelay=2.9,
                             splicedDodgeAnims=dodgeAnims, showDamageExtraTime=2.4)
...
def doJargon(attack):
    ...
    toonTrack = getToonTrack(attack, damageDelay=damageDelay, splicedDamageAnims=damageAnims, dodgeDelay=dodgeDelay,
                             splicedDodgeAnims=dodgeAnims, showMissedExtraTime=1.6, showDamageExtraTime=0.7)
...
def doGuiltTrip(attack):
    ...
    def getPowerTrack(effect, suit=suit, battle=battle):
        partTrack = Sequence(Wait(0.7), Func(battle.movie.needRestoreParticleEffect, effect), Func(effect.start, suit),
                             Wait(0.4), LerpPosInterval(effect, 1.0, Point3(0, 15, 0.4)),
                             LerpFunctionInterval(effect.setAlphaScale, fromData=1, toData=0, duration=0.4),
                             Func(effect.cleanup), Func(battle.movie.clearRestoreParticleEffect, effect))
        return partTrack
...
def doConserve(attack):
    ...
    sprayEffect2 = toontown.battle.movies.BattleParticles.createParticleEffect(file='legaleseSpray')
    sprayEffect3 = toontown.battle.movies.BattleParticles.createParticleEffect(file='legaleseSpray')
    color = Vec4(0.466, 0.474, 1.0, 1.0)
    toontown.battle.movies.BattleParticles.setEffectTexture(sprayEffect1, 'legalese-hc', color=color)
    toontown.battle.movies.BattleParticles.setEffectTexture(sprayEffect2, 'legalese-qpq', color=color)
    toontown.battle.movies.BattleParticles.setEffectTexture(sprayEffect3, 'legalese-vd', color=color)
    partDelay = 1.3
    ...
    cog = suit
    repairSfx = base.loader.loadSfx("phase_5/audio/sfx/SA_repair.ogg")
    repairSeqHeal = Sequence(Wait(3), Func(cog.setHP, cog.getHP() + dmg), Func(repairSfx.play), Func(cog.updateHealthBar, 0), Func(cog.showHpText, dmg))
    damageAnims = []
    damageAnims.append(['cringe',
                        1e-05,
                        0.3,
    ...
    damageAnims.append(['cringe', 1e-05, 0.3])
    toonTrack = getToonTrack(attack, damageDelay=damageDelay, splicedDamageAnims=damageAnims, dodgeDelay=dodgeDelay,
                             dodgeAnimNames=['cringe'], showMissedExtraTime=0.8)
    return Parallel(repairSeqHeal, suitTrack, toonTrack, sprayTrack1, sprayTrack2, sprayTrack3)
...
def doPeckingOrder(attack):
    ...
    toonTrack = getToonTrack(attack, damageDelay=4.2, splicedDamageAnims=damageAnims, dodgeDelay=2.8,
                             dodgeAnimNames=['sidestep'], showMissedExtraTime=1.1)
...
def doCigarSmoke(attack):
    ...
    posPoints = [Point3(-0.4, -0.8, -0.2), VBase3(0, 180, 0)]
    scale = Point3(10.0, 10.0, 10.0)
    scaleDelay = 0.2
    cigarTrack = Sequence(Wait(0.2), Func(__showProp, cigar, suit.getRightHand(), posPoints[0], posPoints[1]),
                          LerpScaleInterval(cigar, scaleDelay, scale, startScale=MovieUtil.PNT3_ZERO))
    cigarTrack.append(Wait(5))
    cigarTrack.append(Func(MovieUtil.removeProp, cigar))
    toonTrack = getToonTrack(attack, partDelay + 0.8, ['conked'], partDelay - 0.2, ['duck'])
    headParts = toon.getHeadParts()
    torsoParts = toon.getTorsoParts()
    legsParts = toon.getLegsParts()
    ...
    if dmg > 0:
        ...
        colorTrack.append(resetColor(legsParts))
        colorTrack.append(Func(battle.movie.clearRestoreColor))
        return Parallel(suitTrack, toonTrack, cigarTrack, partTrack, colorTrack)
    else:
        return Parallel(suitTrack, toonTrack, cigarTrack, partTrack)


def doSongAndDance(attack):
    suit = attack['suit']
    suitTrack = getSuitAnimTrack(attack)
    soundTrack = getSoundTrack('target_happydance.ogg', delay=0.2, node=suit)
    toonTracks = getToonTracks(attack, 4, ['cringe'], 4, ['applause'])
    return Parallel(suitTrack, toonTracks, soundTrack)
...
def doThrowBook(attack):
    ...
    propDelay = 0.6
    holdDelay = 0.2
    airLaunchDuration = 0.6
    airMoveDuration = 0.4
    throwDuration = 1.5
    hitDelay = suitDelay + propDelay + holdDelay + throwDuration
    dodgeDelay = suitDelay + propDelay + holdDelay + (throwDuration / 2.0)
    paper = globalPropPool.getProp('lawbook')
    suitTrack = getSuitTrack(attack)
    posPoints = [Point3(-0.3, -0.35, 0), VBase3(-180, 0, -65)]
    paperTrack = Sequence(
        getPropAppearTrack(paper, suit.getRightHand(), posPoints, propDelay, Point3(1, 1, 1), scaleUpTime=0.5))
    paperTrack.append(Wait(suitDelay))
    hitPoint = toon.getPos(battle)
    hitPoint.setX(hitPoint.getX() + 1.2)
    hitPoint.setY(hitPoint.getY() + 1.5)
    ...
    paperTrack.append(Func(paper.wrtReparentTo, battle))
    paperTrack.append(getThrowTrack(paper, hitPoint, duration=throwDuration, parent=battle))
    paperTrack.append(Wait(airLaunchDuration))
    paperTrack.append(LerpPosInterval(paper, airMoveDuration, movePoint))
    spinTrack = Sequence(Wait(propDelay + suitDelay + holdDelay),
                         LerpHprInterval(paper, throwDuration / 2.0, Point3(0, 120, 0)),
                         LerpHprInterval(paper, throwDuration / 2.0, Point3(0, 360, 0)))
    sizeTrack = Sequence(Wait(propDelay + suitDelay + holdDelay + airLaunchDuration),
                         LerpScaleInterval(paper, throwDuration - airLaunchDuration, Point3(6, 6, 6)), Wait(0.95),
    ...
    damageAnims.extend(getSplicedLerpAnims('slip-forward', 0.31, 0.95, startTime=1.2))
    damageAnims.append(['slip-forward', 0.01, 1.51])
    toonTrack = getToonTrack(attack, hitDelay, splicedDamageAnims=damageAnims, dodgeDelay=dodgeDelay,
                             dodgeAnimNames=['sidestep'], showDamageExtraTime=0.4, showMissedExtraTime=1.3)

    throwSound = getSoundTrack('General_throw_miss.ogg', propDelay + suitDelay + holdDelay, node=suit)

    if dmg > 0:
        hitSound = getSoundTrack('AA_drop_safe_miss.ogg', delay=hitDelay, node=suit)
        return Parallel(suitTrack, toonTrack, propTrack, throwSound, hitSound)
    else:
        return Parallel(suitTrack, toonTrack, propTrack, throwSound)
...
def doDigUp(attack):
    ...
    dmg = target['hp']
    suitTrack = getSuitTrack(attack)
    digSound = getSoundTrack('burrow.ogg', delay=0.8, node=suit)
    numberNames = ['one',
                   'two',
                   'three',
                   'four',
    ...
    damageAnims.append(['slip-forward',
                        ...
                        0.6,
                        1.2])
    toonTrack = getToonTrack(attack, 4, splicedDamageAnims=damageAnims, dodgeDelay=4, dodgeAnimNames=['sidestep'])
    if dmg > 0:
        hitSound = getSoundTrack('MG_cannon_hit_dirt.ogg', delay=4.56, node=suit)
        return Parallel(suitTrack, numberSprayTracks, numberTracks, toonTrack, digSound, hitSound)
    else:
        return Parallel(suitTrack, numberSprayTracks, numberTracks, toonTrack, digSound)


def doWhistleBlow(attack):
    suit = attack['suit']
    ...
    whistleHoldTrack = Sequence(Wait(0.5),
                                Func(__showProp, whistle, suit.getRightHand(), whistleTransforms[0],
                                     whistleTransforms[1], whistleTransforms[2]),
                                Parallel(whistleScaleTrack, whistleMoveTrack))
    toonTrack = getToonTrack(attack, 1.65, ['conked'], 0.2, ['duck'])
    soundTrack = getSoundTrack('AA_sound_whistle.ogg', delay=1.6, node=suit)
    return Sequence(Parallel(suitTrack, whistleHoldTrack, toonTrack, soundTrack),
                    Func(MovieUtil.removeProp, whistle))
