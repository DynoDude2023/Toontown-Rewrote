...
from mechanics import RegularCogCalcAI
...
class BattleCalculatorAI:
    ...
    def assignV2(self, suit):
        self.V2Cogs[suit] = suit.getSkeleRevives()
    
    def generateCogCalc(self, suit):    
        cogCalc = RegularCogCalcAI
        self.cogCalcDict[suit.getDoId()] = cogCalc.CogCalcAI(self.battle, suit)
    ...
    def calcV2Cogs(self):
        for suit in self.battle.activeSuits:
            if suit.getSkeleRevives() and not suit.getSkelecog():
                self.V2Cogs[suit] = 1
            else:
                self.V2Cogs[suit] = 0
            ...
            if atkTrack == LURE:
                ...
                if attack[TOON_TRACK_COL] == NPCSOS and lureDidDamage != 1 or attack[TOON_TRACK_COL] == PETSOS:
                    ...
                    for suit in self.battle.activeSuits:
                        ...
                        self.assignV2(suit)
                        self.calcV2Cogs()
                        subtraction = suit.getActualLevel() * 1.5
                        isSuitV2 = self.V2Cogs[suit]
                        #if the suit's v2.0 in self.V2Cogs, then we need to subtract the subtraction var from the damage.
                        if isSuitV2:    
                            attackDamage -= subtraction
                        else:
    ...
    def __calcSuitAtkType(self, attackIndex):
        ...
        if theSuit.dna.name == 'gh' and theSuit.getHired():
            atk = 4
        else:
    ...
    def __applySuitAttackDamages(self, attackIndex):
        ...
        if self.APPLY_HEALTH_ADJUSTMENTS:
            for t in self.battle.activeToons:
                ...
                if suit.dna.name != 'gh' and not suit.getHired():
                ...
                if suit.dna.name == 'gh' and suit.getHired():
                    toon = self.battle.getToon(t)
                    toon.toonUp(15)

    def __suitCanAttack(self, suitId):
        suit = self.battle.findSuit(suitId)
        if self.__combatantDead(suitId, toon=0) or self.__suitIsLured(suitId) or suit.getHired() == 1 and suit.dna.name != 'gh' or self.__combatantJustRevived(suitId):
            return 0
        return 1
        ...
        for i in xrange(len(self.battle.suitAttacks)):
            if i < len(self.battle.activeSuits):
                ...
                if allTargetsDead:
                    ...
                    attack = self.battle.suitAttacks[i]
                suit = self.battle.findSuit(suitId)
                if self.__attackHasHit(attack, suit=1) or suit.dna.name == 'gh' and suit.getHired():
                    print('Heya!')
