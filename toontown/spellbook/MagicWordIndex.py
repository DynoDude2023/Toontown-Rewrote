...
class MaxToon(MagicWord):
    ...
    def handleWord(self, invoker, avId, toon, *args):
        ...
        toon.b_setRewardHistory(Quests.HIDEOUT_TIER, toon.getRewardHistory()[1])
...
class DistrictId(MagicWord):
    aliases = ["disId", "GiveMEIDAHHHH"]
    desc = "Abort any minigame you are currently in."
    execLocation = MagicWordConfig.EXEC_LOC_SERVER

    def handleWord(self, invoker, avId, toon, *args):
        print(simbase.air.districtId)
...
class printCameraPosHpr(MagicWord):
    desc = "Prints the camera position and hpr."
    execLocation = MagicWordConfig.EXEC_LOC_CLIENT
...
class SetPinkSlips(MagicWord):
    aliases = ["hirepapers", "sethires", "hires"]
    desc = "Sets the target's hires. The default amount is 255."
    execLocation = MagicWordConfig.EXEC_LOC_SERVER
    ...
    def handleWord(self, invoker, avId, toon, *args):
        ...
        return "Restocked {0} hire papper{1} successfully!".format(amt, plural)
