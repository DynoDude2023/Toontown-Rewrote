...
class DistributedSuitPlannerAI(DistributedObjectAI.DistributedObjectAI, SuitPlannerBase.SuitPlannerBase):
    ...
    def assignSuitBuildings(self, numToAssign):
        ...
        while numToAssign > 0:
            ...
            self.notify.debug('Existing buildings are (%s, %s), choosing from (%s, %s), chose %s, %s.' % (self.formatNumSuitsPerTrack(numPerTrack), self.formatNumSuitsPerTrack(numPerHeight), smallestTracks, smallestHeights, buildingTrack, buildingHeight))
            repeat = 1
    ...
    def assignCogdos(self, numToAssign):
        ...
        while numToAssign > 0:
            ...
            self.notify.debug('Assigning cogdo to zone %d' % zoneId)
            numToAssign -= 1
    ...
    def unassignCogdos(self, numToAssign):
        ...
        while numToAssign > 0:
            ...
            self.notify.debug('Unassigning cogdo from zone %s.' % zoneId)
            sp.targetNumCogdos -= 1
            numToAssign -= 1
    ...
    def pickLevelTypeAndTrack(self, level=None, type=None, track=None):
        ...
        if type:
            ...
            elif level < minLevelForType:
                level = minLevelForType
