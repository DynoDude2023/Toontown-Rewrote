    ...
    for key in ModelDict.keys():
        ...
        if flag:
            if base.config.GetBool('want-new-cogs', 0):
                ...
                loader.loadModel('phase_3.5' + model + 'zero').node()
            ...
            if base.config.GetBool('want-new-cogs', 0):
                ...
                loader.unloadModel('phase_3.5' + model + 'zero')
...
class Suit(Avatar.Avatar):
    ...
    def generateSuit(self):
        ...
        if dna.name == 'f':
            ...
            self.handColor = VBase4(0.75, 0.95, 0.75, 1.0)
            self.generateBody()
            self.headTexture = 'stone_cheese.jpg'
            self.generateHead('bigcheese')
            self.setHeight(9.34)
        elif dna.name == 'bf':
            self.scale = 4.0 / cSize
    ...
    def generateAnimDict(self):
        ...
        if not base.config.GetBool('want-new-cogs', 0):
            if self.style.body == 'a':
                animDict['neutral'] = 'phase_4/models/char/tt_a_ene_cga_neutral'
                for anim in SuitsCEOBattle:
                    animDict[anim[0]] = 'phase_12/models/char/tt_a_ene_cga_' + anim[1]

            elif self.style.body == 'b':
                animDict['neutral'] = 'phase_4/models/char/tt_a_ene_cgb_neutral'
                for anim in SuitsCEOBattle:
                    animDict[anim[0]] = 'phase_12/models/char/tt_a_ene_cgb_' + anim[1]

            elif self.style.body == 'c':
                animDict['neutral'] = 'phase_3.5/models/char/tt_a_ene_cgc_neutral'
                for anim in SuitsCEOBattle:
                    animDict[anim[0]] = 'phase_12/models/char/tt_a_ene_cgc_' + anim[1]
    ...
    def makeHired(self):
        ...
        anims['neutral'] = 'phase_5/models/char/tt_a_ene_cg' + string.lower(self.style.body) + '_controlidle'
    ...
    def makeStone(self, modelRoot = None):
        if not modelRoot:
            modelRoot = self
        torsoTex = loader.loadTexture('phase_3.5/maps/stone_blazer.jpg')
        torsoTex.setMinfilter(Texture.FTLinearMipmapLinear)
        torsoTex.setMagfilter(Texture.FTLinear)
        legTex = loader.loadTexture('phase_3.5/maps/stone_leg.jpg')
        legTex.setMinfilter(Texture.FTLinearMipmapLinear)
        legTex.setMagfilter(Texture.FTLinear)
        armTex = loader.loadTexture('phase_3.5/maps/stone_sleeve.jpg')
        armTex.setMinfilter(Texture.FTLinearMipmapLinear)
        armTex.setMagfilter(Texture.FTLinear)
        handT = loader.loadTexture('phase_3.5/maps/stone_hands.jpg')
        handT.setMinfilter(Texture.FTLinearMipmapLinear)
        handT.setMagfilter(Texture.FTLinear)
        modelRoot.find('**/torso').setTexture(torsoTex, 1)
        modelRoot.find('**/arms').setTexture(armTex, 1)
        modelRoot.find('**/legs').setTexture(legTex, 1)
        modelRoot.find('**/hands').setTexture(handT, 1)
        self.leftHand = self.find('**/def_joint_left_hold')
        self.rightHand = self.find('**/def_joint_right_hold')
        self.shadowJoint = self.find('**/def_shadow')
        self.nametagJoint = self.find('**/def_nameTag')
    ...
    def makeWaiter(self, modelRoot = None):
        ...
        self.isWaiter = 1
        dept = self.style.dept
        torsoTex = loader.loadTexture('phase_3.5/maps/waiter_m_blazer_%s.jpg' % dept)
        torsoTex.setMinfilter(Texture.FTLinearMipmapLinear)
        torsoTex.setMagfilter(Texture.FTLinear)
    ...
    def getLoseActor(self):
        ...
        if self.loseActor == None:
            if not self.isSkeleton:
                ...
                loseModel = 'phase_' + str(phase) + filePrefix + 'lose_zero'
                loseAnim = 'phase_' + str(phase) + filePrefix + 'lose'
                self.loseActor = Actor.Actor(loseModel, {'lose': loseAnim})
                loseNeck = self.loseActor.find('**/def_head')
                for part in self.headParts:
                    part.instanceTo(loseNeck)

                if self.isWaiter:
                    self.makeWaiter(self.loseActor)
                elif self.style.name == 'sbc':
                    self.makeStone(self.loseActor)
                else:
                    self.setSuitClothes(self.loseActor)
            else:
                loseModel = 'phase_5/models/char/tt_a_ene_sc' + string.lower(self.style.body) + '_lose_zero'
                filePrefix, phase = TutorialModelDict[self.style.body]
                loseAnim = 'phase_' + str(phase) + filePrefix + 'lose'
                self.loseActor = Actor.Actor(loseModel, {'lose': loseAnim})
        ...
        shadowJoint = self.loseActor.find('**/def_shadow')
    ...
    def makeSkeleton(self):
        model = 'phase_5/models/char/tt_a_ene_sc' + string.lower(self.style.body) + '_zero'
        anims = self.generateAnimDict()
        anim = self.getCurrentAnim()
        dropShadow = self.dropShadow
