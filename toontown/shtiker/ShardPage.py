...
class ShardPage(ShtikerPage.ShtikerPage):
    ...
    def load(self):
        ...
        totalPop_ycoord = shardPop_ycoord - -0.65
    ...
    def makeShardButton(self, shardId, shardName, shardPop):
        ...
        #fg=(1, 0.5, 0.5, 1)
        if shardId in self.safeDistricts:
            shardButtonL = DirectButton(parent=shardButtonParent, relief=None, text=shardName, text_scale=0.06, text_align=TextNode.ALeft, text1_bg=Vec4(0.1, 0.1, 0.8, 1), text2_bg=Vec4(0.1, 0.1, 0.8, 1), text3_fg=Vec4(0.2, 0.2, 0.6, 1), textMayChange=0, command=self.getPopChoiceHandler(shardPop), extraArgs=[shardId])
        else:
