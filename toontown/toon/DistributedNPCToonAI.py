...
class DistributedNPCToonAI(DistributedNPCToonBaseAI):
    ...
    def __init__(self, air, npcId, questCallback = None, hq = 0, accList=[0,0,0,0]):
        DistributedNPCToonBaseAI.__init__(self, air, npcId, questCallback)
        self.hq = hq
        self.npcId = npcId
        self.tutorial = 0
        self.pendingAvId = None
        self.glasses = accList[0]
        self.hat = accList[1]
        self.backpack = accList[2]
        self.shoes = accList[3]
        self.accList = [3, 5]
        return

    def putOnAccessories(self):
        self.sendUpdate('putOnAccessories', [self.glasses, self.hat, self.backpack, self.shoes])
